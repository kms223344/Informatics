...
int main()
{
    ...
    for(double i = 0; i <= 1000; i+=0.01)
    {
        ...
        for (int i = 1; i <= Q; i++)
        {
            if (v[2 * i - 2] - vA[i - 1] <= 1e-9) break;
            vA[i] = f(v[2 * i - 2], vA[i - 1], k);
            v[2 * i - 1] = g(v[2 * i - 2], vA[i - 1], k);

            if (v[2 * i - 1] - vB[i - 1] >= 1e-9) break;
        ...
        for (int i = 1; i <= Q; i++)
        {
            if (v[2 * i - 1]) ans++;// fprintf(output, "%03d-1, %.3lf,%.3lf,%.3lf,%c,|vA(%lf) * %lfm + vB(%lf) * %lfm + v(%lf) * m = %lfm\n", i, vB[i - 1], v[2 * i - 1], vA[i], 'A', vA[i], k, vB[i - 1], k, v[2 * i - 1], (vB[i - 1] + vA[i]) * k + v[2 * i - 1]), ans++;
            if (v[2 * i]) ans++;//fprintf(output, "%03d-2, %.3lf,%.3lf,%.3lf,%c,|vA(%lf) * %lfm + vB(%lf) * %lfm + v(%lf) * m = %lfm\n", i, vB[i], v[2 * i], vA[i], 'B', vA[i], k, vB[i], k, v[2 * i], (vB[i] + vA[i]) * k + v[2 * i]), ans++;
        }
        //fprintf(output, "===%d===\n", ans);
